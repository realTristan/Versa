...
class GlobalFunctions:
    # // Variables from UIMainWindow class
    def init(self, main):
        self.discord_user = main.discord_user
        self.folder_id = main.folder_id
        self.hwid = main.hwid
        self.text_cursor = main.cursor
        return self
    ...
    # // Create a new zip file with logs in it
    def create_zip_file(self):
        zip_file = f"{self.discord_user}_{self.folder_id}"
        shutil.make_archive(zip_file, 'zip', f"{self.folder_id}")
        return zip_file
        
    # // Create Logs file
    def create_logs_file(self):
        with open(f"{self.folder_id}/{time.strftime('%Y.%m.%d')}_logs.txt", 'w') as f:
            creation_time = time.ctime(os.path.getctime(f"{self.folder_id}/{time.strftime('%Y.%m.%d')}_logs.txt"))
    ...
    def create_new_log_files(self, file_id: str):
        file_name: str = time.strftime("%Y.%m.%d.%H.%M") + f"_{file_id}"
        log_file = f"{self.folder_id}/programs_{self.discord_user}_{file_id}.txt"
        ...
        pyautogui.screenshot().save(f"{self.folder_id}/{file_name}.png")
        
        # // Send Logs
        self.text_cursor.insertText(f"\n[LOG] Image/File Saved {file_name} | {time.ctime()}\n")
    
    
    # // Function to write detailed specs
    def write_detailed_pc_specs(self, file: str, file_id: str):
...
''')
    ...
    def send_psutil_logs(self):
        ...
        # // PC Specs
        self.write_detailed_pc_specs(log_file, file_id)
